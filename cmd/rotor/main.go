...
func usage(w io.Writer) {
	...
	fmt.Fprintln(w, "  rotor build [path] [--type <game|model|package>]")
	fmt.Fprintln(w, "                            compile the project to Luau (experimental — writes to tsconfig outDir;")
	...
	fmt.Fprintln(w, "  --type        override project type inference (build only; choices: game, model, package)")
