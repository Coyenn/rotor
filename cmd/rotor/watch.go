...
func (w *treeWatcher) walk(dir string, stamps map[string]fileStamp) {
	...
	for _, entry := range entries {
		...
		// Skip editor write-noise and any file the build itself regenerates
		// every pass; watching the latter makes each build's rewrite
		// self-trigger the next rebuild — an endless loop.
		if isJunkFile(name) || isBuildGeneratedFile(name) {
			continue
		}
...
// flameworkBuildArtifact is rbxts-transformer-flamework's reflection metadata
// cache, rewritten unconditionally next to the project root on every compile.
const flameworkBuildArtifact = "flamework.build"
...
var buildGeneratedFiles = []string{
	...
	flameworkBuildArtifact,
}

// isBuildGeneratedFile reports whether a basename is regenerated by the build
// and therefore must not trigger a rebuild. These are never hand-edited, so a
// name match is the right granularity — same as isJunkFile.
func isBuildGeneratedFile(name string) bool {
	for _, generated := range buildGeneratedFiles {
