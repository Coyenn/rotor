...
// WriteAssetDeclarations is the on-disk editor companion writer: missing →
// written, stale → rewritten, current → untouched, sharing the synthetic
// declaration body so the two are interchangeable.
func TestWriteAssetDeclarations(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, AssetDeclFileName)

	wrote, err := WriteAssetDeclarations(dir)
	if err != nil {
		t.Fatal(err)
	}
	if !wrote {
	...
	if string(data) != AssetDeclFileText {
		t.Error("written content differs from AssetDeclFileText")
	}
	if !strings.Contains(string(data), assetDeclBody) || !strings.Contains(assetDeclText, assetDeclBody) {
	...
	if wrote, err = WriteAssetDeclarations(dir); err != nil || wrote {
...
// rotor-asset.d.ts editor companion (and report the macro usage on the
...
func TestBuildWritesAssetTypesForAssetProjects(t *testing.T) {
	...
	if !result.WroteAssetTypes {
		t.Error("WroteAssetTypes = false, want true on first build")
	}
	if result.WroteLockfile {
		t.Error("WroteLockfile = true on a pure cache-hit build (no upload happened)")
	}
	data, err := os.ReadFile(filepath.Join(dir, AssetDeclFileName))
	if err != nil {
		t.Fatalf("rotor-asset.d.ts not written: %v", err)
	}
	if string(data) != AssetDeclFileText {
		t.Error("rotor-asset.d.ts content differs from AssetDeclFileText")
	}
	...
	if !result.UsesAssetMacro || result.WroteAssetTypes || result.WroteLockfile {
		t.Errorf("rebuild: UsesAssetMacro=%v WroteAssetTypes=%v WroteLockfile=%v; want true,false,false",
			result.UsesAssetMacro, result.WroteAssetTypes, result.WroteLockfile)
	}
}

// Coexistence parity guard: a project carrying the generated on-disk
// rotor-asset.d.ts in its program (tsconfig include) AND using $asset must
// compile without a duplicate-identifier error (the injector skips the
// synthetic declaration) and produce byte-identical output to the baseline.
func TestAssetMacroCoexistsWithOnDiskDeclaration(t *testing.T) {
	baseline := compileRuntimeLibProject(t, "asset_model")

	dir := t.TempDir()
	copyDir(t, filepath.Join("testdata", "asset_model"), dir)
	if _, err := WriteAssetDeclarations(dir); err != nil {
		t.Fatal(err)
	}
	...
	patched := strings.Replace(string(tsconfig), `"include": ["src"]`, `"include": ["src", "rotor-asset.d.ts"]`, 1)
