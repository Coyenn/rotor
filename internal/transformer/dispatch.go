...
func transformStatementDispatch(s *State, node *ast.Node) *luau.List[luau.Statement] {
	...
	// functions, classes, imports/exports, ...) land with Tasks 7-13.
	s.Diags.Add(DiagRotorNotYetSupported(node, kindName(node.Kind)))
	return luau.NewList[luau.Statement]()
}

// ---------------------------------------------------------------------------
// Statements (minimal paths — full ports: Task 7/9)
// ---------------------------------------------------------------------------

// transformVariableStatement ports statements/transformVariableStatement.ts
// transformVariableStatement + transformVariableDeclarationList for the
// Identifier-binding path. Binding patterns and `export let` indirection:
// Task 7 (full port moves this to statements.go).
func transformVariableStatement(s *State, node *ast.Node) *luau.List[luau.Statement] {
	declarationListNode := node.AsVariableStatement().DeclarationList
	// isVarDeclaration: neither Const nor Let flag.
	if declarationListNode.Flags&(ast.NodeFlagsConst|ast.NodeFlagsLet) == 0 {
		s.Diags.Add(DiagNoVar(declarationListNode))
	}

	statements := luau.NewList[luau.Statement]()

	// `export let` requires getModuleIdPropertyAccess indirection
	// (exports.<name> = value): Task 7.
	if ast.HasSyntacticModifier(node, ast.ModifierFlagsExport) && declarationListNode.Flags&ast.NodeFlagsLet != 0 {
		s.Diags.Add(DiagRotorNotYetSupported(node, "exported `let` variable statements"))
		return statements
	}

	for _, declaration := range declarationListNode.AsVariableDeclarationList().Declarations.Nodes {
...
// transformVariableDeclaration, Identifier names only (binding patterns are
// destructuring — later task). The initializer is transformed BEFORE the
// name so references inside the value can be hoisted (upstream comment).
...
// transformVariable ports transformVariableStatement.ts transformVariable for
// the non-exported, non-hoisted path: `local <name> = <right>` as a prereq.
// export-let indirection (getModuleIdPropertyAccess) and checkVariableHoist /
// isHoisted handling: Task 7.
func transformVariable(s *State, identifier *ast.Node, right luau.Expression) luau.AnyIdentifier {
...
// transformExpressionStatement ports statements/transformExpressionStatement.ts
// for the generic path: transform and apply discard semantics. The
// assignment-operator and ++/-- statement specializations land with Task 9.
