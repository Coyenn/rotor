...
// transformExportDeclaration ports the dispatch head of
...
// handleExports collects plain exports via the exports alias map. The
// `export ... from` body (transformExportFrom, digest §2.1) is Phase 3a
// Task 5.
func transformExportDeclaration(s *State, node *ast.Node) *luau.List[luau.Statement] {
	...
	if decl.ModuleSpecifier != nil {
		// TODO(phase-3a Task 5): transformExportFrom.
		s.Diags.Add(DiagRotorNotYetSupported(node, "`export ... from` declarations"))
		return luau.NewList[luau.Statement]()
	}
	return luau.NewList[luau.Statement]()
}
